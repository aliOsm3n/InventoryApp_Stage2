package com.example.aliothman.inventoryapp_stage2.adaptor;
...
public class ProductCursorAdapter extends CursorAdapter  {
    ...
    public void bindView(View view, final Context context, final Cursor cursor) {
        ...
        final int id = cursor.getInt(cursor.getColumnIndex(WardContract.ProductEntry._ID));
        String name = cursor.getString(cursor.getColumnIndex(WardContract.ProductEntry.COLUMN_PRODUCT_NAME));
        int price = cursor.getInt(cursor.getColumnIndex(WardContract.ProductEntry.COLUMN_PRODUCT_PRICE));
        final int quantity = cursor.getInt(cursor.getColumnIndex(WardContract.ProductEntry.COLUMN_PRODUCT_AVAILABLE_QUANTITY));
        ...
        addToCartImageButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Uri currentProductUri = ContentUris.withAppendedId(WardContract.ProductEntry.CONTENT_URI, id);
    ...
    private void addToCart(Context context, Uri uri, int quantity) {
        if (quantity > 0) {
            ...
            values.put(WardContract.ProductEntry.COLUMN_PRODUCT_AVAILABLE_QUANTITY, newAvailableQuantityValue);
